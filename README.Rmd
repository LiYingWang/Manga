...
Manga are Japanese comics or graphic novels that are very popular in the present day, especially in Asia. To see what are the popular manga and how their topics changed over time, I explore the top 100 Manga since 1982 based on the data collected from https://www.anime-planet.com/manga/top-manga. The data includes the top 100 Manga, the year when they were first published, and the tags describing their topics.
...
1. Click on this `launch binder` button <!-- badges: start -->
  [![Launch Rstudio Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/LiYingWang/Manga/master?urlpath=rstudio).
  <!-- badges: end --> to open Rstudio in the browser. It may take a few minutes. 
2. Click on README.Rmd in the lower right pane to open this file, which will show up at the upper left pane.
3. Run the code chunks from the beginning and interact with the first plot by moving your mouse cursor over the point to see the title of each Manga.

```{r plot-top-100, warning=FALSE}
library(plotly)
manga_top_100_inter <-
  df_manga_top_100 %>% 
...
```{r include, echo = FALSE, include= FALSE}
htmlwidgets::saveWidget(top_100_inter, "index.html")
file_path <- file.path(here::here("index.html"))
...
We can see that there are many popular Manga in the recent years after 2012, which shows that the newer the Manga, the more popular it is. However, three out of five of the top five Manga were released before 2002, such as One piece, Berserk, JoJo's bizarre adventure, representing the classic that are still popular even today. 
...
```{r barplot-counts, warning=FALSE}
...
```{r get-tags, message=FALSE, results=FALSE}
...
Drama is the most popular genre, followed by romance, comedy, fantasy, and action. Among those genres, "Seinen" (https://en.wikipedia.org/wiki/Seinen_manga) is a type of Manga aimed at a younger audience especially men that could cover a wide range of topics. "BL" is also a special kind of genre that means Boys Love (https://en.wikipedia.org/wiki/Yaoi).

```{r genre-top-manga, warning=FALSE}
...
The box plot indicates that "Seinen" is the most popular genre over the last 20 years. It is interesting to see that only the two genres, "BL" and "Manga based on a web novel", released after 2012 are popular. Also, the genre "School life" released after 2012 and before 1996 is more popular with a gap from 1997-2011. 

```{r genre-years-relation}
...
The histogram plots show a clear pattern that most genres increase significantly after 2012, especially the genre of drama, comedy, fantasy, and action.

```{r each-genre-per-year, warning=FALSE}
...
```{r subset-time-and-rank, message=FALSE}
library(scales)
...
In the plot, words on the right of the line are the genre that is found more after 2012, and words in red refer to a higher proportion in general. The words near the line are the genre found both in the early and later times with a similar proportion. The result shows that the topic increases and becomes diverse after 2012 with a different focus, such as political, adventure, mature themes, and action. 

```{r proportion-two-periods, warning=FALSE}
